...
public class CurrencyUtil {
	...
	public static Money convertToBYN(Money money) {
		...
		if (money.getCurrency() == Currency.USD) {
			output = new Money(money.getAmount().multiply(BigDecimal.valueOf(USD_TO_BYN)), Currency.BYN);
			return output;
		} else if (money.getCurrency() == Currency.RUR) {			
			output = new Money(money.getAmount().multiply(BigDecimal.valueOf(RUR_TO_BYN)), Currency.BYN);
			return output;
		} else if (money.getCurrency() == Currency.EUR) {			
			output = new Money(money.getAmount().multiply(BigDecimal.valueOf(EUR_TO_BYN)), Currency.BYN);
			return output;
		} 
		else
			return null;
